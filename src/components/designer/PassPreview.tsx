...
interface PassPreviewProps {
  ...
  passId?: string;
  qrToken?: string; // The real unique token to encode in the QR
}
...
export default function PassPreview({
  ...
  // Real QR code as a data URL
  const [qrDataUrl, setQrDataUrl] = useState<string>('');
 
  useEffect(() => {
    // What gets encoded: the qr_token if provided, otherwise the passId
    // This is what gets scanned at the door and checked against the database
  const valueToEncode = qrToken || passId;
   QRCode.toDataURL(valueToEncode, {
  width: 250,
  margin: 2,
  errorCorrectionLevel: 'H', // 🔥 stronger scanning reliability
  color: {
  ...
  // Shared QR block renderer — uses real QR image
  const renderQRBlock = (size: string, bgColor: string, borderColor: string, padding: string = 'p-1.5', radius: string = 'rounded-lg') => (
    <div
  className={`${size} ${bgColor} ${padding} ${radius} border ${borderColor} flex items-center justify-center flex-shrink-0`}
>
  {qrDataUrl ? (
    <img
      src={qrDataUrl}
    ...
    <div className="w-full h-full rounded bg-neutral-200 dark:bg-neutral-700 animate-pulse" />
  )}
</div>
  );
  ...
  switch (template) {
    ...
    case 'minimal': {
      return (
        <div
          ...
          className={`w-full max-w-[420px] bg-white border border-[#E2E2E2] rounded-[20px] shadow-sm text-[#111111] overflow-hidden transition-all duration-300 ${getFontFamilyClass()}`}
        >
          {/* Banner */}
          <div className="w-full h-[120px] bg-neutral-100 flex items-center justify-center relative border-b border-neutral-100">
            {banner_url ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-neutral-400">
                ...
                <span className="text-[10px] font-medium tracking-widest uppercase opacity-50">BANNER</span>
              </div>
            )}
          ...
          <div className="p-6 space-y-5">
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-1.5 text-xs font-semibold tracking-wide text-neutral-500 font-mono">
                <span>{formattedDate(date as string)}</span>
                <span>•</span>
                <span>{time}</span>
              </div>
              <p className="text-xs text-neutral-500 tracking-tight">{venue}</p>
            </div>
            ...
            {/* Attendee */}
            <div className="space-y-1">
              <span className="text-[9px] font-bold tracking-widest uppercase font-mono" style={{ color: brand_color }}>
                TICKET HOLDER
              </span>
            ...
            <div className="flex items-end justify-between gap-4">
              {renderQRBlock('w-[84px] h-[84px]', 'bg-white', 'border-neutral-200', 'p-1.5', 'rounded-lg')}
              <div className="text-right space-y-1">
                <p className="text-[10px] text-neutral-400 uppercase tracking-wider">Organized by</p>
                <p className="text-xs font-medium text-neutral-700">{organizer_name}</p>
                <p className="text-[11px] font-mono font-medium tracking-wider text-neutral-800 bg-neutral-100 px-2 py-0.5 rounded inline-block mt-1">
                  {passId}
                </p>
              </div>
            </div>
    ...
    case 'dark': {
      return (
        <div
          id="pass-card-dark"
          className={`w-full max-w-[420px] bg-[#111111] border border-neutral-800 rounded-[20px] shadow-lg text-[#F2F2F2] overflow-hidden transition-all duration-300 ${getFontFamilyClass()}`}
        >
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 bg-neutral-900/40 p-3 rounded-lg border border-neutral-900/60 font-mono text-xs text-neutral-400">
              <div>
                ...
                <p className="font-semibold text-neutral-300 mt-0.5">{formattedDate(date as string)}</p>
                <p className="text-[11px] text-neutral-400">{time}</p>
              </div>
              <div>
                <p className="text-[9px] text-neutral-500 uppercase tracking-widest">LOCATION</p>
                <p className="font-semibold text-neutral-300 mt-0.5 truncate">{(venue as string).split(',')[0]}</p>
                <p className="text-[11px] text-neutral-400 truncate">{(venue as string).split(',').slice(1).join(',') || ''}</p>
              </div>
            </div>
 
            {/* Attendee */}
            <div className="space-y-1 pl-4 border-l-2" style={{ borderLeftColor: brand_color }}>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-neutral-900">
              {renderQRBlock('w-[88px] h-[88px]', 'bg-neutral-950', 'border-neutral-800', 'p-2', 'rounded-xl')}
    ...
    case 'elegant': {
      return (
        <div
          ...
          className={`w-full max-w-[420px] bg-[#161616] border border-[#2a2a2a] rounded-[24px] shadow-lg text-[#EEEEEE] overflow-hidden transition-all duration-300 ${getFontFamilyClass()}`}
        >
          {banner_url && (
            <div className="w-full h-[90px] overflow-hidden">
              <img src={banner_url} alt="Banner" className="w-full h-full object-cover brightness-75" />
            </div>
          )}
          ...
          <div className="p-7 space-y-7">
            ...
            <div className="text-center space-y-2">
              ...
              <h3 className="text-2xl font-bold tracking-tight text-white uppercase italic leading-none">
                {name}
              </h3>
              <p className="text-xs font-mono text-neutral-400 tracking-widest bg-neutral-900/60 inline-block px-3 py-1 rounded-full border border-neutral-800">
                {formattedDate(date as string)} • {time}
              </p>
            </div>
 
            <div className="border-t border-b border-neutral-900 py-4 text-center">
              <p className="text-[10px] font-semibold tracking-widest uppercase font-mono" style={{ color: brand_color }}>
                PASS HOLDER
              </p>
            ...
            <div className="flex flex-col items-center gap-4 text-center">
              {renderQRBlock('w-[110px] h-[110px]', 'bg-[#111111]', 'border-neutral-800', 'p-2.5', 'rounded-xl')}
              <div className="space-y-1">
    ...
    default: {
      return (
        <div
          id="pass-card-modern"
          className={`w-full max-w-[420px] bg-[#1B1B1B] border border-[#333333] rounded-[28px] shadow-lg text-[#F2F2F2] overflow-hidden transition-all duration-300 relative ${getFontFamilyClass()}`}
        >
          {/* Top Banner */}
          <div className="w-full h-[140px] bg-neutral-900 relative">
            {banner_url ? (
              ...
              <div className="absolute inset-0 bg-neutral-950 flex flex-col items-center justify-center text-neutral-500">
                ...
                <span className="text-[10px] font-semibold tracking-wider opacity-65">EVENT BANNER</span>
              </div>
            )}
            {/* Brand badge */}
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-1.5">
              <span className="text-[10px] font-bold tracking-widest uppercase font-mono" style={{ color: brand_color }}>
                OFFICIAL ENTRY PASS
              </span>
            ...
            <div className="flex justify-between items-center gap-4">
              <div className="space-y-1 flex-1">
                ...
                <div
                  className="inline-block px-2.5 py-0.5 rounded-full text-[10px] font-bold tracking-wider uppercase font-mono"
              ...
              <div className="text-right space-y-0.5 text-xs">
                ...
                <p className="font-semibold text-neutral-300 font-mono mt-0.5 leading-tight">{formattedDate(date as string)}</p>
            ...
            <div className="flex items-center gap-5 justify-between">
              {renderQRBlock('w-20 h-20', 'bg-neutral-900', 'border-neutral-800', 'p-1.5', 'rounded-xl')}
